...
/**
* @file
* Contains Drupal\mallaform\form\MallaForm
*/
...
use Drupal\malla\Controller\MallaEndpoint;
...
* 
*/
class MallaForm extends FormBase
{
...
public function buildForm(array $form, FormStateInterface $form_state) {
  ...
  $tid_planactual = $output_planactual[0]['field_concesion'];
	...
	$term = \Drupal::entityTypeManager()->getStorage('taxonomy_term')->load($linea->tid);
 ...
 $form['filtro']['year'] = array (
  ...
  if (isset($input['op']) && $input['op'] === 'Programas') {
   ...
   $form['table'] = [ 
   	 	  '#type' => 'tableselect',
   		'#description' => '* Debe seleccionar el programa para que sean alamacenado los datos.',
  	    '#header' => $headertotal,
        '#options' => $output3,
        '#empty' => t('Sin contenido.'),
        //'#default_value' => variable_get('table'),
  	    '#multiple' => TRUE,
       	'#prefix' => '<div id="programas-wrapper-subp" "style=margin-left: 0!important;"><h3>* Seleccionar o checkear los programas en los cuales se ingresa datos, de lo contrario no se almacenará la informacón</h3>',
   		'#suffix' => '</div>',
        '#sticky' => TRUE,
        '#attributes' => array(
...
public function saveplan(array &$form, FormStateInterface $form_state) {
       ...
       \Drupal::messenger()->addMessage(
           $this->t("Plan Malla gurdado correctamente")
         );
      ...
      foreach ($form_state->getUserInput()['table'] as $key => $value) {
        ...
        for ($i=1; $i < 13; $i++) { 
           switch ($i) {
            ...
            ${$mes."_proc_mnc_" .$value} = isset($form_state->getUserInput()[$mes."_proc_mnc_".$value]) ? $form_state->getUserInput()[$mes."_proc_mnc_".$value] : 0;
            //echo $mes."_proc_mnc_".$idSubprograma ."=". ${$mes."_proc_mnc_" . $value} ."<br>";

            ${$mes."_proc_mc_" .$value} = isset($form_state->getUserInput()[$mes."_proc_mc_".$value]) ? $form_state->getUserInput()[$mes."_proc_mc_".$value] : 0;
            //echo $mes."_proc_mc_".$idSubprograma ."=". ${$mes."_proc_mc_" . $value} ."<br>";

            ${$mes."_prod_mnc_" .$value} = isset($form_state->getUserInput()[$mes."_prod_mnc_".$value]) ? $form_state->getUserInput()[$mes."_prod_mnc_".$value] : 0;
            //echo $mes."_prod_mnc_".$idSubprograma ."=". ${$mes."_prod_mnc_" . $value} ."<br>";

            ${$mes."_prod_mc_" .$value} = isset($form_state->getUserInput()[$mes."_prod_mc_".$value]) ? $form_state->getUserInput()[$mes."_prod_mc_".$value] : 0;
            ...
            if (!empty(${$mes."_proc_mnc_" .$value}) || !empty(${$mes."_proc_mc_" .$value})) {
              ...
              $node = Node::create(array(
                  ...
                  'uid' => array($user_id),
                  'status' => 1,
                  'field_biblioteca' => array($biblioteca_2),
                  'field_concesion' => $plan,
                  'field_linea' => $idSubprograma,
                  'field_fecha_plan_operativo' => array($newDateString),
                  'field_proc_interna'=> array(intval(${$mes."_proc_mnc_" . $value})), //mpnc, No contratada (Valor de la meta proceso que se va a realizar sin contratar externo)  
                  'field_proc_externo'=> array(intval(${$mes."_proc_mc_" .$value})), //mpc, Contratado (Valor de la meta proceso que se va a realizar contratando externo) 
                  'field_meta_sesiones'=> array(intval(${$mes."_proc_mnc_" . $value}) + intval(${$mes."_proc_mc_" .$value})), // Meta proceso
                  //'field_meta_extension_externa'=> array(${$mes."_prod_mnc_" .$value} + ${$mes."_prod_mc_" .$value}), // Extension proceso
                  'field_prod_interno'=> array(intval(${$mes."_prod_mnc_" .$value})), //mprodnc, No contratada (Valor de la meta producto que se va a realizar sin contratar externo) 
                  'field_prod_externo'=> array(intval(${$mes."_prod_mc_" .$value})), //mprodc, Contratado (Valor de la meta producto que se va a realizar contratando externo)  
                  'field_numero_asistentes'=> array(intval(${$mes."_prod_mnc_" .$value}) + intval(${$mes."_prod_mc_" .$value})), // Meta producto (Automático) 
      ...
      foreach ($form_state->getValues() as $key => $value) {
          //drupal_set_message($key . ':values ' . $value) ;
        }

...
class ActividadTerritorialForm extends FormBase {

  public function getFormId() {
      return 'actividadesterritoriales_form'; //nombremodule_nombreformulario
    }
  ...
  $lineas['_none'] = "Seleccionar una línea";
  ...
  //$tid_linea_agenda = \Drupal::entityTypeManager()->getStorage('taxonomy_term')->load($linea->tid)->get('field_tid_linea_agenda')->getValue()[0]['value'];
    ...
    //$categoria_actividad = $termino->get('field_category_activities')->getValue(); //field_categoria_actividad->value;
    $categoria_actividad = $termino->get('field_categoria_actividad')->getValue(); //
  ...
  */
  ...
  $form['filtro']['actions'] = [
        ...
        'submit' => [
            '#type' => 'submit',
            '#value' => $this->t('Ingresar Actividad Territorial'),
        ],
    ...
    $subprogramas = array();
      ...
      if ($form_state->getValue('programas') != 0) {
        ...
        foreach ($terms as $key => $value) {
          ...
          foreach ($categoria_actividad as $cat => $value_cat) {
          	$categoria = isset($value_cat) ? $value_cat['target_id'] : "";
